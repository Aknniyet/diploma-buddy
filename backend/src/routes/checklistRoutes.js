...
async function ensureChecklist(userId) {
  const existing = await query(`SELECT COUNT(*)::int AS count FROM adaptation_checklist_tasks WHERE user_id = $1`, [userId]);
...
router.get("/", authenticate, async (req, res) => {
  try {
    ...
    return res.json(result.rows.map((item) => ({ ...item, completed: item.is_completed })));
  } catch (error) {
...
router.patch("/:taskId/toggle", authenticate, async (req, res) => {
  try {
    ...
    return res.json({ task: { ...result.rows[0], completed: result.rows[0].is_completed } });
  } catch (error) {
